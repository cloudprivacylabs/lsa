    ...
    for key,val in yaml_queries.items():
        for s in val:
            ...
            for idx_start in result:
                for i,c in enumerate(s[idx_start:]):
                    if c == "}":
                        # +1, exclude {
                        placeholders.append(s[idx_start+1:i])
                        break
...
main("http://localhost:8000") # http://localhost:8000?id=gender&concept_id=123&concept_name=blah