...
func IngestBytes(ctx *ls.Context, baseID string, input []byte, schemaRoot graph.Node, builder ls.GraphBuilder) (graph.Node, error) {
	node, err := jsonom.Unmarshal(input, ctx.GetInterner())
	if err != nil {
		return nil, err
	}
	parser := Parser{
	...
	pd, err := parser.ParseDoc(ctx, baseID, node, schemaRoot)
	if err != nil {
		return nil, err
	}
