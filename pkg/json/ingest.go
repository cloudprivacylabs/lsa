...
func (ingester *Ingester) ingest(input jsonom.Node, path []interface{}, schemaNode ls.Node) (ls.Node, []deferredProperty, error) {
...
func (ingester *Ingester) ingestPolymorphicNode(input jsonom.Node, path []interface{}, schemaNode ls.Node) (ls.Node, []deferredProperty, error) {
	var dp []deferredProperty
	node, err := ingester.Polymorphic(path, schemaNode, func(p []interface{}, optionNode ls.Node) (ls.Node, error) {
...
func (ingester *Ingester) ingestObject(input *jsonom.Object, path []interface{}, schemaNode ls.Node) (ls.Node, []deferredProperty, error) {
	// An object node
...
func (ingester *Ingester) ingestArray(input *jsonom.Array, path []interface{}, schemaNode ls.Node) (ls.Node, []deferredProperty, error) {
	elementsNode := ingester.GetArrayElementNode(schemaNode)
	elements := make([]ls.Node, 0, input.Len())

	dp := make([]deferredProperty, 0)
	for index := 0; index < input.Len(); index++ {
		childNode, prop, err := ingester.ingest(input.N(index), append(path, index), elementsNode)
		if err != nil {
...
func (ingester *Ingester) ingestValue(input *jsonom.Value, path []interface{}, schemaNode ls.Node) (ls.Node, []deferredProperty, error) {
	var value interface{}
	var typ string
	if input.Value() != nil {
...
func findStringPath(path []interface{}) string {
	for i := len(path) - 1; i >= 0; i-- {
		if str, ok := path[i].(string); ok {
			return str
		}
	}
	if len(path) > 0 {
