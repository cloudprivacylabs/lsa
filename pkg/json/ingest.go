...
type Ingester struct {
	...
	NodeIDGeneratorFunc func(input interface{}, path []interface{}, schemaNode *ls.SchemaNode) string
}
...
func (ingester *Ingester) DefaultNodeIDGenerator(input interface{}, path []interface{}, schemaNode *ls.SchemaNode) string {
...
func (ingester *Ingester) Ingest(target *digraph.Graph, input interface{}) (ls.DocumentNode, error) {
	if input == nil {
		return nil, nil
	}

	path := make([]interface{}, 0, 16)
	var root *ls.SchemaNode
	if ingester.Schema != nil {
		root = ingester.Schema.GetRoot()
	}
...
func (ingester *Ingester) ingestObject(target *digraph.Graph, input map[string]interface{}, path []interface{}, schemaNode *ls.SchemaNode) (ls.DocumentNode, error) {
	// An object node
	nextNodes := make(map[string]*ls.SchemaNode)
	...
	if schemaNode != nil {
		...
		addNextNode := func(node *ls.SchemaNode) error {
			key, _ := node.Properties[ingester.KeyTerm].(string)
			if len(key) == 0 {
...
func (ingester *Ingester) ingestArray(target *digraph.Graph, input []interface{}, path []interface{}, schemaNode *ls.SchemaNode) (ls.DocumentNode, error) {
	var elements *ls.SchemaNode
	if schemaNode != nil {
...
func (ingester *Ingester) ingestValue(target *digraph.Graph, input interface{}, path []interface{}, schemaNode *ls.SchemaNode) (ls.DocumentNode, error) {
	if schemaNode != nil {
...
func (ingester *Ingester) generateID(input interface{}, path []interface{}, schemaNode *ls.SchemaNode) string {
