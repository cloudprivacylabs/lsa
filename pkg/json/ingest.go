...
func IngestBytes(context *ls.Context, ingester *Ingester, targetGraph graph.Graph, baseID string, input []byte) (graph.Node, error) {
	return IngestStream(context, ingester, targetGraph, baseID, bytes.NewReader(input))
}
...
func IngestStream(context *ls.Context, ingester *Ingester, targetGraph graph.Graph, baseID string, input io.Reader) (graph.Node, error) {
	...
	return ingester.Ingest(context, targetGraph, baseID, node)
}
...
func (ingester *Ingester) Ingest(context *ls.Context, targetGraph graph.Graph, baseID string, input jsonom.Node) (graph.Node, error) {
	ingester.PreserveNodePaths = true
	path, root := ingester.Start(context, baseID)
	dn, dp, err := ingester.ingest(context, targetGraph, input, path, root)
	if err != nil {
		return nil, err
	}
	if len(dp) > 0 {
	...
	ingester.Finish(context, dn, nil)
	return dn, err
}

func (ingester *Ingester) ingest(context *ls.Context, targetGraph graph.Graph, input jsonom.Node, path ls.NodePath, schemaNode graph.Node) (graph.Node, []deferredProperty, error) {

	validate := func(node graph.Node, dp []deferredProperty, err error) (graph.Node, []deferredProperty, error) {
		if err != nil {
			return nil, nil, err
		}
		if err := ingester.Validate(context, node, schemaNode); err != nil {
			return nil, nil, err
		}
		return node, dp, nil
	}
	...
	if schemaNode != nil || !ingester.OnlySchemaAttributes {
		if schemaNode != nil && schemaNode.GetLabels().Has(ls.AttributeTypePolymorphic) {
			return validate(ingester.ingestPolymorphicNode(context, targetGraph, input, path, schemaNode))
		}
		switch next := input.(type) {
		case *jsonom.Object:
			return validate(ingester.ingestObject(context, targetGraph, next, path, schemaNode))
		case *jsonom.Array:
			return validate(ingester.ingestArray(context, targetGraph, next, path, schemaNode))
		}
		return validate(ingester.ingestValue(context, targetGraph, input.(*jsonom.Value), path, schemaNode))
	}
	return nil, nil, nil
}

func (ingester *Ingester) ingestPolymorphicNode(context *ls.Context, targetGraph graph.Graph, input jsonom.Node, path ls.NodePath, schemaNode graph.Node) (graph.Node, []deferredProperty, error) {
	var dp []deferredProperty
	g, node, err := ingester.Polymorphic(context, targetGraph, path, schemaNode, func(g graph.Graph, p ls.NodePath, optionNode graph.Node) (graph.Node, error) {
		n, x, err := ingester.ingest(context, g, input, p, optionNode)
		if err != nil {
			return nil, err
		}
		if x != nil {
...
func (ingester *Ingester) ingestObject(context *ls.Context, targetGraph graph.Graph, input *jsonom.Object, path ls.NodePath, schemaNode graph.Node) (graph.Node, []deferredProperty, error) {
	// An object node
	// There is a schema node for this node. It must be an object
	nextNodes, err := ingester.GetObjectAttributeNodes(context, schemaNode)
	if err != nil {
		return nil, nil, err
	}
	...
	for i := 0; i < input.Len(); i++ {
		...
		childNode, props, err := ingester.ingest(context, targetGraph, keyValue.Value(), append(path, keyValue.Key()), schNode)
		if err != nil {
	...
	node, err := ingester.Object(context, targetGraph, path, schemaNode, elements, ObjectTypeTerm)
	if err != nil {
		return nil, nil, err
	}
...
func (ingester *Ingester) ingestArray(context *ls.Context, targetGraph graph.Graph, input *jsonom.Array, path ls.NodePath, schemaNode graph.Node) (graph.Node, []deferredProperty, error) {
	elementsNode := ingester.GetArrayElementNode(schemaNode)
	elements := make([]graph.Node, 0, input.Len())

	dp := make([]deferredProperty, 0)
	for index := 0; index < input.Len(); index++ {
		childNode, prop, err := ingester.ingest(context, targetGraph, input.N(index), path.AppendInt(index), elementsNode)
		if err != nil {
	...
	node, err := ingester.Array(context, targetGraph, path, schemaNode, elements, ArrayTypeTerm)
	if err != nil {
		return nil, nil, err
	}
	dp, err = processDeferredProperties(dp, node, func(ID string) (graph.Node, error) {
...
func (ingester *Ingester) ingestValue(context *ls.Context, targetGraph graph.Graph, input *jsonom.Value, path ls.NodePath, schemaNode graph.Node) (graph.Node, []deferredProperty, error) {
	...
	node, err := ingester.Value(context, targetGraph, path, schemaNode, value, typ)
	return node, nil, err
}
