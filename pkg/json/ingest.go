...
func IngestStream(ingester *Ingester, target *digraph.Graph, baseID string, input io.Reader) (ls.Node, error) {
	...
	return ingester.Ingest(target, baseID, node)
}
...
func (ingester *Ingester) Ingest(target *digraph.Graph, baseID string, input jsonom.Node) (ls.Node, error) {
...
func (ingester *Ingester) ingest(target *digraph.Graph, input jsonom.Node, path []interface{}, schemaNode ls.Node) (*addedNode, error) {
	...
	return validate(ingester.ingestValue(target, input.(*jsonom.Value), path, schemaNode))
}

func (ingester *Ingester) ingestPolymorphicNode(target *digraph.Graph, input jsonom.Node, path []interface{}, schemaNode ls.Node) (*addedNode, error) {
...
func (ingester *Ingester) ingestObject(target *digraph.Graph, input *jsonom.Object, path []interface{}, schemaNode ls.Node) (*addedNode, error) {
	// An object node
...
func (ingester *Ingester) ingestArray(target *digraph.Graph, input *jsonom.Array, path []interface{}, schemaNode ls.Node) (*addedNode, error) {
	...
	for index := 0; index < input.Len(); index++ {
		childNode, err := ingester.ingest(target, input.N(index), append(path, index), elements)
		if err != nil {
...
func (ingester *Ingester) ingestValue(target *digraph.Graph, input *jsonom.Value, path []interface{}, schemaNode ls.Node) (*addedNode, error) {
	if schemaNode != nil {
