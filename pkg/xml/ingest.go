...
func IngestStream(ingester *Ingester, targetGraph graph.Graph, baseID string, input io.Reader) (graph.Node, error) {
	...
	return ingester.IngestDocument(targetGraph, baseID, decoder)
}
...
func (ingester *Ingester) IngestDocument(targetGraph graph.Graph, baseID string, decoder *xml.Decoder) (graph.Node, error) {
	...
	for !done {
		...
		case xml.StartElement:
			...
			rootSeen = true
			rn, err := ingester.parseElement(targetGraph, token, path, decoder, schemaRoot, wsFacet)
			if err != nil {
				return nil, err
			}
			rootNode = rn.node
...
func (ingester *Ingester) parseElement(targetGraph graph.Graph, data xml.StartElement, path ls.NodePath, decoder *xml.Decoder, schemaNode graph.Node, wsFacet WhitespaceFacet) (parsedElement, error) {
	...
	for index, attribute := range data.Attr {
		...
		attrNode, err := ingester.Value(targetGraph, append(path, fmt.Sprintf("attr-%d", index)), attrSchema, attribute.Value)
		if err != nil {
			return parsedElement{}, err
		}
	...
	for {
		...
		case xml.StartElement:
			...
			node, err := ingester.parseElement(targetGraph, token, path.AppendInt(index), decoder, elemSchema, wsFacet)
			if err != nil {
				return parsedElement{}, err
			}
			if node.node != nil {
		...
		case xml.EndElement:
			...
			case (hasNonTextNodes && hasTextNodes) || // Mixed content
				...
				for i, x := range children {
					node := x.node
					if node == nil {
						node, err = ingester.Value(targetGraph, path, schemaNode, string(children[i].token.(xml.CharData)))
						if err != nil {
							return parsedElement{}, err
						}
				...
				if schemaNode != nil && schemaNode.GetLabels().Has(ls.AttributeTypeArray) {
					newNode, err = ingester.Array(targetGraph, path, schemaNode, append(attributeNodes, childNodes...))
				} else {
					newNode, err = ingester.Object(targetGraph, path, schemaNode, append(attributeNodes, childNodes...))
				}
			case !hasNonTextNodes && !hasTextNodes: // No content
				if schemaNode != nil {
					if schemaNode.GetLabels().Has(ls.AttributeTypeValue) {
						newNode, err = ingester.Value(targetGraph, path, schemaNode, nil)
					} else if schemaNode.GetLabels().Has(ls.AttributeTypeArray) {
						newNode, err = ingester.Array(targetGraph, path, schemaNode, nil)
					} else {
						newNode, err = ingester.Object(targetGraph, path, schemaNode, nil)
					}
				} else {
					newNode, err = ingester.Value(targetGraph, path, schemaNode, nil)
				}
			...
			case !hasNonTextNodes && hasTextNodes: // Value, there is only one child
				...
				node, err := ingester.Value(targetGraph, path, schemaNode, str)
				if err != nil {
					return parsedElement{}, err
				}
				if schemaNode != nil && schemaNode.GetLabels().Has(ls.AttributeTypeArray) {
					newNode, err = ingester.Array(targetGraph, path, schemaNode, []graph.Node{node})
				} else {
					newNode = node
				}
			}
