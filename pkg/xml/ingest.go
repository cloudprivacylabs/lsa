...
func (ingester *Ingester) IngestDocument(context *ls.Context, baseID string, decoder *xml.Decoder) (graph.Node, error) {
	...
	for !done {
		...
		case xml.StartElement:
			...
			ictx = &ctx
			ctx.SourcePath.AppendString(token.Name.Local)
			_, rootNode, err = ingester.Object(ctx)
			if err != nil {
				return nil, err
			}
			_, err := ingester.parseElement(ctx.NewLevel(rootNode), token, decoder, wsFacet)
			if err != nil {
				return nil, err
			}
			done = true
...
func (ingester *Ingester) parseElement(context ls.IngestionContext, data xml.StartElement, decoder *xml.Decoder, wsFacet WhitespaceFacet) (parsedElement, error) {
	...
	schemaNodes, err := ls.GetObjectAttributeNodes(schemaNode)
	if err != nil {
	...
	for index, attribute := range data.Attr {
		...
		_, attrNode, err := ingester.Value(context.New(fmt.Sprintf("attr-%d", index), attrSchema), attribute.Value)
		if err != nil {
			return parsedElement{}, err
		}
		if attrNode != nil {
