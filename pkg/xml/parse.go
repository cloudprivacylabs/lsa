...
func (ing *Parser) parseValue(ctx parserContext, element *xmlElement) (*ParsedDocNode, error) {
	...
	if ctx.schemaNode != nil {
		...
		if len(pvalue) > 0 {
			...
			if ok {
				...
				ret.properties[ls.AttributeNameTerm] = ls.StringPropertyValue(ls.GetTermInfo(ls.AttributeNameTerm).Term, element.name.Local)
				if len(element.name.Space) > 0 {
					ret.properties[NamespaceTerm] = ls.StringPropertyValue(ls.GetTermInfo(NamespaceTerm).Term, element.name.Space)
				}
				return ret, nil
			}
			return nil, nil
	...
	for _, attribute := range element.attributes {
		...
		ret.properties[makeFullName(attribute.name)] = ls.StringPropertyValue(ls.GetTermInfo(makeFullName(attribute.name)).Term, attribute.value)
	}
...
func (ing *Parser) attributes(ctx parserContext, element *xmlElement, childSchemaNodes []graph.Node) ([]ls.ParsedDocNode, error) {
	...
	for _, attribute := range element.attributes {
		...
		if len(attribute.name.Space) > 0 {
			attrNode.properties[NamespaceTerm] = ls.StringPropertyValue(ls.GetTermInfo(NamespaceTerm).Term, ctx.context.GetInterner().Intern(attribute.name.Space))
		}
		attrNode.properties[ls.AttributeNameTerm] = ls.StringPropertyValue(ls.GetTermInfo(ls.AttributeNameTerm).Term, ctx.context.GetInterner().Intern(attribute.name.Local))
...
func (ing *Parser) parseArray(ctx parserContext, element *xmlElement) (*ParsedDocNode, error) {
	...
	for index, child := range element.children {
		...
		if newNode != nil {
			newNode.properties[ls.AttributeIndexTerm] = ls.IntPropertyValue(ls.GetTermInfo(ls.AttributeIndexTerm).Term, index)
